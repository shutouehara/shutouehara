        ...
        <article className="p-3 p-sm-5">
          <div className="py-3">
            ...
            <ul className="ul-list gap">
              {data.allMicrocmsBlog.edges.map(({node}) => (
                <li key={node.blogId} className="shadow">
                  <CardOriginal linkParent="blogs" linkTo={node.blogId} imgSrc={node.blog_thumbnail.url} title={node.blog_title} description={node.blog_desc} moreText="詳しくみる→" />
                </li>
              ))}
            </ul>
          </div>
          <ul className="ps-0">
            <li className="py-3 list-unstyled">
              <h2 className="mt-5 mb-4 pb-2 border-bottom border-dark">うぇぶはん.com</h2>
              <p className="mb-5">主にデザインのインプット＆アウトプットにまとめているブログです。テレビ局のウェブ班として、テレビ番組HPなどのデザインが多いです。<br />WordPressに慣れ親しむために初めて制作したブログサイトです。</p>
              <div className="shadow p-4 d-inline-block" style={{ border: '8px solid #03092e' }} >
            ...
            <li className="py-3 list-unstyled">
              <h2 className="mt-5 mb-4 pb-2 border-bottom border-dark">おきなわフリー写真　ゆいまーる</h2>
